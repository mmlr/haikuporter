...
class Source(object):
		...
		try:
			...
			for patch in self.patches:
				...
				if getOption('noGitRepo'):
					...
					output = check_output(['patch', '--ignore-whitespace',
							'-p1', '-i', patch], cwd=self.sourceDir) \
								.decode('utf-8')
					info(output)
				else:
					if patch.endswith('.patchset'):
						info('Applying patchset "%s" ...' % patch)
						output = check_output(['git', 'am',
								'--ignore-whitespace', '-3', '--keep-cr',
								patch], cwd=self.sourceDir,
							env=self.gitEnv).decode('utf-8')
						info(output)
					else:
						info('Applying patch "%s" ...' % patch)
						output = check_output(['git', 'apply',
								'--ignore-whitespace', '-p1', '--index', patch],
	...
	def commitPatchPhase(self):
		...
		info('Committing changes done in patch function for '
			  + self.fetchTargetName)
		output = check_output(['git', 'commit', '-a', '-q', '-m',
				'patch function'], cwd=self.sourceDir, env=self.gitEnv) \
			.decode('utf-8')
		info(output)
	...
	def _initImplicitGitRepo(self):
		...
		info(check_output(['git', 'config', 'gc.auto', '0'],
				cwd=self.sourceDir).decode('utf-8'))
		...
		info(check_output(['git', 'symbolic-ref', 'HEAD',
					'refs/heads/haikuport'], cwd=self.sourceDir)
				.decode('utf-8'))
